...
// vrmlNodeからTextureCoordinateオブジェクトを作る
CX3DTextureCoordinateNode::CX3DTextureCoordinateNode(jobject vrmlNode)
{
	if (!vrmlNode)
	{
		m_vrmlNode = NULL;
	...
	if (tmpNode)
	{
		...
		if (nodeName && strcmp(nodeName, "TextureCoordinate")==0)
		{
...
void CX3DTextureCoordinateNode::print(int indent)
{
	...
	if (nodeName)
	{
		...
		for (i=0; i<n; i++)
		{
			...
			m_point.getValue(i).getValue(x, y);
